...
# from .utils.utils import plot_gripper_pro_max, batch_rgbdxyz_2_rgbxy_depth, get_batch_key_points, \
#     batch_key_points_2_tuple, framexy_depth_2_xyz, batch_framexy_depth_2_xyz, center_depth, key_point_2_rotation, \
#     batch_center_depth, batch_framexy_depth_2_xyz, batch_key_point_2_rotation
...
class RectGrasp():
    ...
    def __repr__(self):
        return 'Rectangle Grasp: score:{}, height:{}, open point:{}, center point:{}, object id:{}'.format(self.score,
                                                                                                           self.height,
                                                                                                           self.open_point,
                                                                                                           self.center_point,
                                                                                                           self.object_id)
...
class RectGraspGroup():
    ...
    def add(self, rect_grasp):
        ...
        **Input:**
        ...
        self.rect_grasp_group_array = np.concatenate(
            (self.rect_grasp_group_array, rect_grasp.rect_grasp_array.reshape((-1, RECT_GRASP_ARRAY_LEN))))
        return self
    ...
    def batch_get_key_points(self):
        ...
        **Output:**
        ...
        upper_points = np.einsum('ij,njk->nik', counter_clock_wise_rotation_matrix,
    ...
    def to_grasp_group(self, camera, depths, depth_method=batch_center_depth):
        ...
        **Output:**
        ...
        upper_points_xyz = np.array(
            batch_framexy_depth_2_xyz(upper_points[:, 0], upper_points[:, 1], depths_2d, camera)).T
        ...
        grasp_group.grasp_group_array = copy.deepcopy(
            np.hstack((scores, widths, heights, depths, rotations, translations, object_ids))).astype(np.float64)
        return grasp_group
...
# from graspclutter6dAPI.utils.utils import generate_views, transform_points
...
# from graspclutter6dAPI.utils.rotation import batch_viewpoint_params_to_matrix
