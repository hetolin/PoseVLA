...
LOG_DIR="/home/henryhyyu/RoboTwin/eval_result/auto_eval_20260703_100223"
...
for log_file in "$LOG_DIR"/*.log; do
    ...
    if [ "$score" != "N/A" ]; then
        ...
        if [ -n "$score_num" ]; then
            ...
            if float_ge "$score_num" "50.0"; then
                ((success_count++))
            else
                ((failed_count++))
            fi
...
for task_name in "${task_names[@]}"; do
    ...
    if [ "$score" = "N/A" ]; then
        ...
        score_num=$(echo "$score" | sed 's/[^0-9.]//g')
        if float_ge "$score_num" "80.0"; then
            # Green for >= 80%
            printf "%-30s \033[32m%10.1f\033[0m%%\n" "$task_name" "$score_num"
        elif float_ge "$score_num" "50.0"; then
            # Yellow for >= 50%
...
for task_name in "${task_names[@]}"; do
    ...
    if [ "$score" != "N/A" ]; then
        score_num=$(echo "$score" | sed 's/[^0-9.]//g')
        if float_lt "$score_num" "50.0"; then
